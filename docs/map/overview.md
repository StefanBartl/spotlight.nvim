...
**3 modules** · 4 namespaces · 22 helper files
...
flowchart LR
  ...
  nlua_spotlight_health_lua --> nlua_spotlight_sets_lua
  nlua_spotlight_health_lua --> nlua_spotlight_util
  nlua_spotlight_map_lua --> nlua_spotlight_config
  nlua_spotlight_map_lua --> nlua_spotlight_core
  ...
  nlua_spotlight_sets_lua --> nlua_spotlight_core
  nlua_spotlight_sets_lua --> nlua_spotlight_persist_lua
  nlua_spotlight_sets_lua --> nlua_spotlight_util
  ...
  nlua_spotlight_winopt_lua --> nlua_spotlight_core
  nlua_spotlight_yank_lua --> nlua_spotlight_config
  nlua_spotlight_yank_lua --> nlua_spotlight_core
```
...
|---|---|---|---|
| `spotlight` | One entry point that wires configuration and exposes every user-facing action. | 33 | [src](../../lua/spotlight/init.lua) |
...
| &nbsp;&nbsp;`spotlight.config` | Deep-merges user options over `spotlight.config.DEFAULTS`, validates the handful of values that can break rendering or matching if wrong, and exposes a single… | 11 | [src](../../lua/spotlight/config/init.lua) |
| &nbsp;&nbsp;`core` |  |  |  |
| &nbsp;&nbsp;`ui` |  |  |  |
| &nbsp;&nbsp;`util` |  |  |  |
